...
Feature: Admin API — Runtime Configuration
  ...
  #
  # Fixture notes:
  #   - service-offering.loire.signed.jwt  → type gx:ServiceOffering (direct base-class match)
  #   - digital-service-offering.loire.signed.jwt → type gx:DigitalServiceOffering
  #     (additional_root under ServiceOffering base class in gaia-x-2511/framework.yaml)
  #   - participant.loire.signed.jwt → type gx:LegalPerson (Participant base class — must stay active)
  #
  ...
  @baseline @cfg.strict @cfg.test-sig
  Scenario: Base class disabled — ServiceOffering credential rejected (direct match)
    Given Gaia-X trust framework is enabled
      And base class ServiceOffering of bundle gaia-x-2511 is disabled
  ...
  @baseline @cfg.strict @cfg.test-sig
  Scenario: Base class disabled — DigitalServiceOffering credential rejected (additional_root)
    # gx:DigitalServiceOffering is an additional_root of ServiceOffering
    # in the gaia-x-2511 framework.yaml; disabling the base class must also block this type.
    Given Gaia-X trust framework is enabled
      And base class ServiceOffering of bundle gaia-x-2511 is disabled
  ...
  @baseline @cfg.strict @cfg.test-sig
  Scenario: Base class disabled — other base class (Participant) still accepted
    # disabling ServiceOffering must not affect the Participant base class.
    # gx:LegalPerson is the root type for the Participant base class in gaia-x-2511.
    Given Gaia-X trust framework is enabled
      And base class ServiceOffering of bundle gaia-x-2511 is disabled
  ...
  @baseline @cfg.strict @cfg.test-sig
  Scenario: Re-enable base class — previously rejected credential now accepted
    # after re-enabling ServiceOffering, the same credential succeeds.
    Given Gaia-X trust framework is enabled
      And base class ServiceOffering of bundle gaia-x-2511 is disabled
      And base class ServiceOffering of bundle gaia-x-2511 is re-enabled
      And credential from fixture "loire/valid/service-offering.loire.signed.jwt" is not uploaded
    When add credential from fixture "loire/valid/service-offering.loire.signed.jwt" with content-type "application/vc+jwt"
    Then get http 201:Created code
      And credential from fixture "loire/valid/service-offering.loire.signed.jwt" is not uploaded
  ...
  @baseline @cfg.default
  Scenario: GET admin trust-frameworks returns base-class state for gaia-x-2511
    # GET /admin/trust-frameworks lists bundles with base classes.
    # Default state: all base classes enabled (true).
    Given Gaia-X trust framework is enabled
    When request admin trust frameworks
    Then get http 200:Success code
      And admin trust frameworks response includes bundle "gaia-x-2511" with base class "ServiceOffering" enabled
